...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 *      (final 로 인해 값 변경 불가, static으로 인해 클래스명만으로 접근 가능하며,
		 ...
		 * - 서브클래스에서 인터페이스를 상속받아 구현해야할 경우 implements 키워드 사용
		 *   => 클래스는 상속받아 확장(extends)의 개념이 강하지만,
		 *      인터페이스는 상속받아 추상메서드를 구현(implements) 하는 개념이 강함
		 * - 클래스는 다중 상속이 불가능 하지만, 인터페이스는 다중 구현(implements) 가능함
...
//		MyInterface.NUM1 = 10;	// 변경 불가
//		MyInterface.NUM2 = 20;	// 변경 불가
//		MySubClass.NUM1 = 10;	// 변경 불가
//		msc.NUM1 = 10;			// 변경 불가
		
		// 인터페이스는 인스턴스 생성은 부가능하지만, 참조변수 타입으로 사용 가능
		...
		//    인터페이스의 모든 메서드는 참조영역이 축소 되더라도 호출 가능함(공통 메서드)
		mi.mehtod1();
		mi.method2();
...
class MySubClass implements MyInterface {
	...
	// => 인터페이스에서 method2() 의 접근제한자를 생략 했지만, 자동으로 public이 붙으므로
...
//	}
	...
	@Override
	public void method2() {	// 접근제한자 public만 사용 가능(부모가 public 이므로)
