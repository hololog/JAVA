
public class Ex3 {
	public static void main(String[] args) {
		/*
		 * 상수(Constant)
		 * - 변수의 반대 개념으로 항상 고정된 데이터(변하지 않는 데이터)
		 * - 실제 사용하는 데이터(상수)를 리터럴(Literal)이라고도 한다.
		 * ex) 정수형 리터럴 1, 실수형 리터럴 3.14
		 * 
		 * */
		
		System.out.println(1);	  // 정수형 (int) 리터럴
		System.out.println(3.14); // 실수형 (double) 리터럴
		System.out.println('A');  // 문자형 리터럴 (작은따움표로 둘러싼 1개의 문자)
		System.out.println(' ');
		
		System.out.println(true); // 논리형 (boolean) 리터럴(true or false)
//		System.out.println(TRUE); // 대문자는 오류발생!
		System.out.println(100L); // 정수형 (long형) 리터럴
		System.out.println(1.5F); // 실수형 (float) 리터럴
		System.out.println("Hellow, Wolrd!");  // 문자열형 리터럴
		System.out.println("");
		
		
	}
}





