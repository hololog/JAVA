...
//		gc.setData("홍길동"); // 컴파일에러 발생! Integer 타입만 전달 가능!
		...
		// => GenericClass 클래스 내의 모든 T 타입이 Double 타입으로 변경됨
		gc2.setData(3.14);
		...
		// => GenericClass 클래스 내의 모든 T 타입이 String 타입으로 변경됨
		gc3.setData("홍길동");
		String strName = gc3.getData();
		System.out.println(strName);
		
		// 4. 제네릭 타입 T 를 Person 타입으로 지정
		//    Setter 호출하여 Person 객체 전달하고
		//    Getter 호출하여 Person 객체 Person p로 리턴받아 데이터 출력
		...
		GenericClass gc6 = new GenericClass();	// 경고 메시지 표시됨
		GenericClass<Object> gc7 = new GenericClass<Object>();	// 경고 메세지 미표시됨
