...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * => 또한 생성된 공간은 각각 0부터 배열크기-1까지 인덱스 자동으로 부여됨
		 * 
		 * < 배열 접근 기본 문법 >
		 * 변수명[인덱스]
		 * => 해당 변수가 가리키는 배열 공간의 지정된 인덱스 번호에 해당하는 요소에 접근
		 * => 변수명은 배열 이름(= 아파트 이름)이고, 인득스는 배열요소 위치(아파트 호수)
		 * 
		 ...
		 * int[]     arr  = {1, 2, 3, 4, 5, ..., n}
		 * => 데이터 갯수 n만큼의 배열 공간이 자동으로 생성되고, 해당 값으로 초기화됨.
		 *    또한, 생성된 배열의 크기가 고정이므로 차후 더많은 데이터 저장은 불가능
		 * => 만약, 배열 선언을 먼저 수행한 후 변도로 한꺼번에 초기화 하는 경우
		 *    변수명 = new 데이터타입[]{데이터1, 데이터2, ..., 데이터n}
		 *    arr  = new int[]{1, 2, 3, 4, 5, ..., n}
...
//		=> 5개의 값을 기본형 변수 대신 배열에 저장
		...
		// 위의 반복문처럼 사용해도 되지만 배열 크기가 변하면 코드도 변경되여야함.
