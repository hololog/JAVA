...
public class Ex4 {
	...
	public static void main(String[] args) {
		...
		System.out.println("s1.substring(5) : " + s1.substring(5)); // 5번인덱스 부터 끝까지 추출
		// => 5번 인덱스 위치가 대문자 P이므로 P부터 끝까지 모든 문자열(Programming) 추출됨
		
		// 2) substring(int beginIndex, int endIndex) : 시작인덱스부터 끝인덱스-1 까지 추출
		System.out.println("s1.substring(5, 12) : " + s1.substring(5, 12)); // 5 ~ 11번 까지 추출
		// => 5번 인덱스 부터 12-1(11번) 인덱스까지 추출되므로 Program 추출됨
		...
		System.out.println(s1.lastIndexOf("a")); // 10flxjsehla
		// => "Java Programming" 문자열에서 뒷쪽부터 "a" 를 탐색할 경우 10번 인덱스의 a를
		//    먼저 만나므로 해당 인덱스(10)를 리턴
		// => indexOf() 를 사용하면 Java의 두번째 글자 "a"가 탐색되므로 인덱스 1 리턴됨
		...
		// => 문자열 입력 시 의도치 않은 불필요한 공백이 삽인되는 것을 방지할 때 사용
		// String s2 = "         아이티윌      부산 교육센터       ";
